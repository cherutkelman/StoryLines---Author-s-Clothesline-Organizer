...
const MapsManager: React.FC<MapsManagerProps> = ({
  ...
  return (
    <div className="h-full flex flex-col bg-[var(--bg-page)]">
      <div className="flex-shrink-0 bg-[var(--bg-card)] border-b border-[var(--color-border)] px-6 py-2 flex items-center justify-between shadow-sm z-10">
        <div className="flex items-center gap-1 bg-[var(--color-secondary)]/50 p-1 rounded-xl border border-[var(--color-border)]/50 overflow-x-auto scrollbar-hide">
          <button 
            onClick={() => handleTabChange('characterDiagram')}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold transition-all ${activeTab === 'characterDiagram' ? 'bg-[var(--color-primary)] text-white shadow-md' : 'text-[var(--color-primary)]/60 hover:text-[var(--color-primary)] hover:bg-[var(--color-secondary)]'}`}
          >
            <Users size={14} />
          ...
          <button 
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold transition-all ${activeTab === 'worldMaps' ? 'bg-[var(--color-primary)] text-white shadow-md' : 'text-[var(--color-primary)]/60 hover:text-[var(--color-primary)] hover:bg-[var(--color-secondary)]'}`}
          >
            <MapIcon size={14} />
            <span>מפות עולם</span>
          </button>
          <button 
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold transition-all ${activeTab === 'mindMaps' ? 'bg-[var(--color-primary)] text-white shadow-md' : 'text-[var(--color-primary)]/60 hover:text-[var(--color-primary)] hover:bg-[var(--color-secondary)]'}`}
          >
            <Share2 size={14} />
        ...
        {activeTab === 'worldMaps' && currentMapId && (
          <div className="flex items-center gap-3">
             ...
             <input 
               ...
               className="text-sm font-bold bg-transparent border-none focus:ring-0 p-0 text-[var(--text-accent)] handwritten text-xl"
               placeholder="שם המפה..."
             />
          </div>
        )}
              ...
              <div className="flex-1 p-12 overflow-y-auto">
                <div className="max-w-4xl mx-auto">
                  <div className="flex items-center justify-between mb-8">
                    <h2 className="text-2xl font-bold text-[var(--text-accent)] handwritten text-4xl">המפות שלי</h2>
                    <button 
                      onClick={addMap}
                      className="flex items-center gap-2 px-6 py-3 bg-[var(--color-primary)] text-white rounded-2xl font-bold hover:opacity-90 transition-all shadow-lg"
                    >
                      <Plus size={20} />
                      <span>מפה חדשה</span>
                    </button>
                  ...
                  {maps.length === 0 ? (
                    <div className="bg-[var(--bg-card)]/50 border-2 border-dashed border-[var(--color-border)] rounded-[3rem] p-20 text-center">
                      <div className="w-20 h-20 bg-[var(--color-secondary)] text-[var(--color-primary)] rounded-full flex items-center justify-center mx-auto mb-6">
                        <MapIcon size={40} />
                      </div>
                      <h3 className="text-xl font-bold text-[var(--text-accent)] mb-2">אין עדיין מפות</h3>
                      <p className="text-[var(--text-main)]/60 max-w-sm mx-auto">צור מפה חדשה כדי להתחיל לאייר את העולם שלך, להוסיף בתים, הרים ושמות למקומות.</p>
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                      {maps.map(map => (
                        <div 
                          ...
                          className="group bg-[var(--bg-card)] rounded-[2rem] border border-[var(--color-border)]/50 p-6 shadow-sm hover:shadow-xl transition-all cursor-pointer relative overflow-hidden"
                        >
                          <div className="aspect-video bg-[var(--color-secondary)]/30 rounded-xl mb-4 flex items-center justify-center border border-[var(--color-border)]/50 overflow-hidden">
                            {map.backgroundImage ? (
                              <img src={map.backgroundImage} alt={map.name} className="w-full h-full object-cover opacity-50" />
                            ) : (
                              <MapIcon size={32} className="text-[var(--color-primary)]/20" />
                            )}
                          </div>
                          <div className="flex items-center justify-between">
                            <h4 className="font-bold text-[var(--text-accent)] handwritten text-2xl truncate flex-1">{map.name}</h4>
                            <button 
          ...
          <div className="h-full flex flex-col">
            {!currentMindMapId ? (
              <div className="flex-1 p-12 overflow-y-auto">
                <div className="max-w-4xl mx-auto">
                  <div className="flex items-center justify-between mb-8">
                    <h2 className="text-2xl font-bold text-[var(--text-accent)] handwritten text-4xl">מפות החשיבה שלי</h2>
                    <button 
                      onClick={addMindMap}
                      className="flex items-center gap-2 px-6 py-3 bg-[var(--color-primary)] text-white rounded-2xl font-bold hover:opacity-90 transition-all shadow-lg"
                    >
                  ...
                  {mindMaps.length === 0 ? (
                    <div className="bg-[var(--bg-card)]/50 border-2 border-dashed border-[var(--color-border)] rounded-[3rem] p-20 text-center">
                      ...
                      <h3 className="text-xl font-bold text-[var(--text-accent)] mb-2">אין עדיין מפות חשיבה</h3>
                      <p className="text-[var(--text-main)]/60 max-w-sm mx-auto">צור מפת חשיבה חדשה כדי לארגן את הרעיונות שלך, לחבר בין מושגים ולתכנן את העלילה.</p>
                    </div>
                  ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                      {mindMaps.map(map => (
                        ...
                        >
                          <div className="aspect-video bg-[var(--color-secondary)]/30 rounded-xl mb-4 flex items-center justify-center border border-[var(--color-border)]/50 overflow-hidden">
                            <Share2 size={32} className="text-[var(--color-primary)]/20" />
                          </div>
                          <div className="flex items-center justify-between">
                            <h4 className="font-bold text-[var(--text-accent)] handwritten text-2xl truncate flex-1">{map.name}</h4>
                            <button 
